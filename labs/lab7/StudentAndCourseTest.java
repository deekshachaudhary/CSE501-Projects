...
//			assertEquals("createBaby should not alter the parents", a + " " + b, s.getName());
			assertEquals("createBaby should not alter the parents", " ", s.getName());
...
//			assertEquals("createBaby should not alter the parents", a2 + " " + b2, ss.getName());
			assertEquals("createBaby should not alter the parents", " ", ss.getName());
	...
	public void testAverageGPA() {
		for (int j = 0; j < 100; j++) {
			...
			for(int i = 0; i < s; i++) {
				...
				double g = Math.round(Math.random() * 4000) / 1000.0;
				gpaSum += g;
//				System.out.println(g + " " +s2.submitGrade(g, 1));
				s2.submitGrade(g, 1);
				c.addStudent(s2);
